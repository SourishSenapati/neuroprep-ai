...
// Insert new session
export async function insertSession(userId, mode) {
    const pool = getPool();
    // Increment user session count
    await pool.query('UPDATE users SET sessions_count = COALESCE(sessions_count, 0) + 1 WHERE id = $1', [userId]);
    const result = await pool.query('INSERT INTO sessions (user_id, mode) VALUES ($1, $2) RETURNING id', [userId, mode]);
...
export async function updateSession(sessionId, data) {
    ...
    await pool.query(`UPDATE sessions SET ${updates.join(', ')} WHERE id = $${paramCount}`, values);
}
// Add biometric data point
export async function addBiometric(sessionId, biometric) {
    const pool = getPool();
    await pool.query(`UPDATE sessions 
     SET biometrics = array_append(biometrics, $1::jsonb) 
     WHERE id = $2`, [JSON.stringify(biometric), sessionId]);
}
// Insert session response
export async function insertSessionResponse(sessionId, data) {
    const pool = getPool();
    await pool.query(`INSERT INTO session_responses 
...
export async function updateReadiness(userId) {
    const pool = getPool();
    ...
    await pool.query('UPDATE users SET readiness = $1, updated_at = NOW() WHERE id = $2', [JSON.stringify(readiness), userId]);
    return readiness;
}
// Detect stress patterns
export async function detectStressPatterns(userId) {
    const pool = getPool();
    const result = await pool.query('SELECT * FROM detect_stress_patterns($1)', [userId]);
    return result.rows;
}
// Get dashboard data
export async function getDashboardData(userId) {
    ...
    // Get recent sessions
    const sessionsResult = await pool.query(`SELECT id, mode, score, neural_resilience, ts 
     FROM sessions 
     WHERE user_id = $1 
     ORDER BY ts DESC 
     LIMIT 10`, [userId]);
    // Get stress history from biometrics
    const stressResult = await pool.query(`SELECT 
    ...
    const modeResult = await pool.query(`SELECT mode FROM sessions WHERE user_id = $1 GROUP BY mode ORDER BY COUNT(*) DESC LIMIT 1`, [userId]);
    const primaryMode = modeResult.rows[0]?.mode || 'standard';
    const benchmarkResult = await pool.query('SELECT * FROM benchmarks WHERE mode = $1', [primaryMode]);
    // Detect patterns
...
export async function generateLongitudinalInsights(userId, openaiKey) {
    const pool = getPool();
    ...
    const trendData = {
        ...
        stress_patterns: sessions.flatMap(s => (s.biometrics || []).map((b) => b.stressLevel)),
...
export async function insertSingularitySession(userId, mode, quantumSeed, initialBiometrics = []) {
    const pool = getPool();
    ...
    await pool.query(`INSERT INTO singularity_sessions 
...
export async function updateSingularitySession(sessionId, data) {
    const pool = getPool();
    ...
    if (data.biometrics) {
        ...
        if (getRes.rows.length > 0 && getRes.rows[0].multi_biometrics) {
            try {
                ...
                if (!Array.isArray(currentBiometrics))
                    currentBiometrics = [];
            }
            catch (e) { }
        }
    ...
    if (updates.length === 0)
        return;
    values.push(sessionId);
    await pool.query(`UPDATE singularity_sessions SET ${updates.join(', ')} WHERE id = $${paramCount}`, values);
}
// Get Singularity Session Data (for Rift Manifold)
export async function getSingularitySession(sessionId) {
    const pool = getPool();
    const result = await pool.query('SELECT * FROM singularity_sessions WHERE id = $1', [sessionId]);
    return result.rows[0];
}
// Get Rift Benchmarks
export async function getRiftBenchmarks(mode) {
    const pool = getPool();
    const result = await pool.query('SELECT * FROM rift_bench WHERE mode = $1', [mode]);
    return result.rows[0];
}
// Predict Rifts (Mock AI/SQL Hybrid)
export async function predictRifts(userId) {
    ...
    const pool = getPool();
    const history = await pool.query('SELECT multi_biometrics FROM singularity_sessions WHERE user_id = $1 ORDER BY ts DESC LIMIT 5', [userId]);
