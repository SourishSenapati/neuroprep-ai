...
export interface QuestionTemplate {
  ...
  difficulty: number;
  topic: string;
  type: 'conceptual' | 'coding' | 'design' | 'debugging' | 'behavioral' | 'system' | 'optimization' | 'security' | 'scalability' | 'architecture';
  tags: string[];
 ...
 */
function generateQuestionId(role: string, topic: string, pattern: string, seed: number): string {
...
export function generateUniqueQuestion(
  ...
  const types = Object.keys(QUESTION_PATTERNS) as Array<keyof typeof QUESTION_PATTERNS>;
  const type = types[rng.nextInt(types.length)] as any;
 ...
 */
function getRoleTopics(role: string): Record<string, string[]> {
...
export class QuestionBankManager {
  private askedQuestions: Map<string, Set<string>> = new Map();
  private questionSeeds: Map<string, QuantumRNG> = new Map();
  private questionHistory: Map<string, QuestionTemplate[]> = new Map();
  private topicCoverage: Map<string, Map<string, number>> = new Map();
  private difficultyProgression: Map<string, number[]> = new Map();
  
  getNextQuestion(sessionId: string, role: string, difficulty: number): QuestionTemplate {
  ...
  private adaptDifficulty(progression: number[], targetDifficulty: number): number {
    if (progression.length < 3) return targetDifficulty;
  ...
  private isTopicOverCovered(coverage: Map<string, number>, topic: string): boolean {
  ...
  getTopicCoverage(sessionId: string): Record<string, number> {
    const coverage = this.topicCoverage.get(sessionId);
    return coverage ? Object.fromEntries(coverage) : {};
  }
  
  getDifficultyProgression(sessionId: string): number[] {
    return this.difficultyProgression.get(sessionId) || [];
  }
  
  getQuestionHistory(sessionId: string): QuestionTemplate[] {
...
export class QuestionBankPerformanceMonitor {
  private static instance: QuestionBankPerformanceMonitor;
  private generationTimes: number[] = [];
  private collisionCounts: number[] = [];
  
  static getInstance(): QuestionBankPerformanceMonitor {
    if (!this.instance) {
